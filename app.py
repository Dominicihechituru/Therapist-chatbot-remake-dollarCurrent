...
def rex():
    ...
    if request.method == 'POST':
        ...
        if prompt_count >= 200 and not check_subscription_status(subscription_code):
            return jsonify({'answer': "NOTIFICATION!: Sorry, you've hit your daily free message limit, or your subscription has expired. <a href='https://new-pidgin-chatbot.onrender.com/payment'>Click here to continue with a weekly or monthly plan</a>, or check back tomorrow for another free trial."}), 200
        if prompt_count >= 200 and check_subscription_status(subscription_code):
